...
public class ModelMapper implements Mapper {
    ...
    private final ConcurrentHashMap<String, ThreadLocal<Unmarshaller>> unmarshallerCache =
            new ConcurrentHashMap<>();

    @SneakyThrows
    public PageObject<Media> mapMedias(InputStream jsonStream) {
        GraphQlResponse<PageObject<Media>> medias =
                mapObject(jsonStream, "me/postaddict/instagram/scraper/model/medias.json");
    ...
    public Media mapMedia(InputStream jsonStream) {
        GraphQlResponse<Media> graphQlResponse =
                mapObject(jsonStream, "me/postaddict/instagram/scraper/model/media-by-url.json");
    ...
    public PageObject<Comment> mapComments(InputStream jsonStream) {
        GraphQlResponse<PageObject<Comment>> comments =
                mapObject(jsonStream, "me/postaddict/instagram/scraper/model/comments.json");
    ...
    public Location mapLocation(InputStream jsonStream) {
        GraphQlResponse<Location> graphQlResponse =
                mapObject(jsonStream, "me/postaddict/instagram/scraper/model/location.json");
    ...
    public Tag mapTag(InputStream jsonStream) {
        GraphQlResponse<Tag> graphQlResponse =
                mapObject(jsonStream, "me/postaddict/instagram/scraper/model/tag.json");
    ...
    public PageObject<Account> mapFollow(InputStream jsonStream) {
        GraphQlResponse<PageObject<Account>> follow =
                mapObject(jsonStream, "me/postaddict/instagram/scraper/model/follow.json");
        return follow.getPayload();
    }

    public PageObject<Account> mapFollowers(InputStream jsonStream) {
        GraphQlResponse<PageObject<Account>> followers =
                mapObject(jsonStream, "me/postaddict/instagram/scraper/model/followers.json");
    ...
    public ActionResponse<Comment> mapMediaCommentResponse(InputStream jsonStream) {
        ActionResponse<Comment> commentActionResponse = mapObject(jsonStream,
                "me/postaddict/instagram/scraper/model/mediaCommentResponse.json",
    ...
    public PageObject<Account> mapLikes(InputStream jsonStream) {
        GraphQlResponse<PageObject<Account>> likesResponse =
                mapObject(jsonStream, "me/postaddict/instagram/scraper/model/mediaLikes.json");
        return likesResponse.getPayload();
    }

    public ActivityFeed mapActivity(InputStream jsonStream) {
        GraphQlResponse<ActivityFeed> activityFeed =
                mapObject(jsonStream, "me/postaddict/instagram/scraper/model/activity.json");
        return activityFeed.getPayload();
    }

    private void updateMediaTime(Media media) {
        if (media.getTakenAtTimestamp() != null
                && media.getTakenAtTimestamp() < MediaUtil.INSTAGRAM_BORN_YEAR) {
    ...
    private void updateCommentTime(Comment comment) {
        if (comment.getCreatedAt() != null
                && comment.getCreatedAt() < MediaUtil.INSTAGRAM_BORN_YEAR) {
    ...
    public Account mapAccount(InputStream jsonStream) {
        GraphQlResponse<Account> graphQlResponse =
                mapObject(jsonStream, "me/postaddict/instagram/scraper/model/account-binding.json",
    ...
    private Node getDomModel(InputStream jsonStream) throws java.io.IOException {
        ...
        return new DomTransformer(new NopTypeConverter())
                .transform(Collections.singletonMap("root", jsonMap));
    }
    ...
    public boolean isAuthenticated(InputStream jsonStream) {
        ...
        return jsonMap.get(AUTHENTICATED_FIELD) instanceof Boolean
                && (boolean) jsonMap.get(AUTHENTICATED_FIELD);
    }
    ...
    protected <T> T mapObject(InputStream jsonStream, String mappingFile, Class rootClass) {
        try {
            ...
            return (T) unmarshaller.get().unmarshal(new StreamSource(jsonStream), rootClass)
                    .getValue();
        } catch (JAXBException e) {
            throw new IllegalArgumentException(e);
        } catch (Exception e) {
    ...
    protected Unmarshaller getUnmarshaller(String mappingFile) {
        ...
        JAXBContext jaxbContext =
                DynamicJAXBContextFactory.createContextFromOXM(classLoader, properties);
